...
迭代法：前序遍历是中左右，每次先处理的是中间节点，那么先将根节点放入栈中，然后将右孩子加入栈，再加入左孩子。
...
迭代法：迭代法是在前序的基础上更改来的，在前序遍历中，先右再左，而后序则是先左后右，然后将res数组进行一个翻转，就可以得到左右中的顺序。
...
使用优先级队列，大堆顶的构建，遍历数组将数值存入大堆顶中，自然会按照从大到小的顺序排列，最后通过k去pop，得到第k个最大元素。

```c++
class Solution {
public:
    int findKthLargest(vector<int>& nums, int k) {
        priority_queue<int,vector<int>,less<int>> big_heap;  //大堆顶
...
public:
    vector<vector<int>> generate(int numRows) {
        ...
        {
            res[i].resize(i+1);// 将当前行大小调整为i+1
            res[i][0] = res[i][i] = 1;// 设置当前行的第一个和最后一个元素为1
...
public:
    int rob(vector<int>& nums) {
        ...
        if(nums.size() == 1)    return nums[0];     //如果nums长度为1，返回nums
...
我们可以枚举这些数，假设当前枚举到 j，那么我们还需要取若干数的平方，构成 i−j*j。此时我们发现该子问题和原问题类似，只是规模变小了。这符合了动态规划的要求，于是我们可以写出状态转移方程。

f[i]=1+ min(f[i-j*j])   min代表历史最小
````c++
class Solution {
public:
    ...
    string longestPalindrome(string s) {
        ...
        for (int L = 2; L <= n; L++) // 字串长度从2开始
        {
            for (int i = 0; i < n; i++) {
                ...
                if (s[i] != s[j]) // 如果两端不同则不是回文
                {
                    dp[i][j] = false;
                } else {
                    if (j - i < 3) // 如果长度为1和2必然是回文德
                        dp[i][j] = true;
                    else // 大于等于3，判断内部各缩一个是不是回文
                        dp[i][j] = dp[i + 1][j - 1];
                }

                if (dp[i][j] && j - i + 1 > maxlen) // 如果是回文就记录起始和结束位置
                {
                    maxlen = j - i + 1;
                    begin = i;
                }
        ...
        return s.substr(begin, maxlen); // 返回裁剪后的字符串
    }
};
```
...
我的解法，一开始用暴力直接超时，更换了解法，先计算两数之间的差放入一个容器中，然后寻找最大子区间和
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        {
            nums[i] = prices[i+1] - prices[i];      //获取差值
        }
...
public:
    bool canJump(vector<int>& nums) {
        ...
        for(int i = 0;i < n;i++)
        {
            if(i <= rightmost){ //判断是否能够到达
                rightmost = max(rightmost,i+nums[i]);
                if(rightmost >= n-1)
                    return true;    //判断是否能够到达结尾
            }
        }
...
1. 判断数组长度，如果为0，直接返回false

2. 计算数组总和，判断能否被三整除，如果不能直接返回false

3. 遍历数组，累加到平均值，标志位加一

4. 遍历完之后，判断标志位是否大于等于3（排除0，0，0，0的情况）
...
public:
    bool canThreePartsEqualSum(vector<int>& arr) {
        ...
        if(sum%3 != 0)  return false;       //总和不能被3整除，说明不能分

        int aver = sum/3;                   //三等分总和值
        int sum2 = 0;
        int count = 0;
